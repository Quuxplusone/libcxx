...
void test_ctor_throws()
{
    ...
    try {
        ...
        std::uninitialized_relocate(values, values + N, It(p));
        assert(false);
    } catch (...) {}
...
void test_counted()
{
    ...
    auto ret = std::uninitialized_relocate(It(values), It(values + 1), FIt(p));
    assert(ret == FIt(p +1));
    assert(Counted::constructed == 1);
    assert(Counted::count == 1);
    assert(p[0].value == 1);
    assert(values[0] == 0);
    ret = std::uninitialized_relocate(It(values+1), It(values+N), FIt(p+1));
    assert(ret == FIt(p + N));
...
void test_unique_ptr_forward()
{
    ...
    auto result = std::uninitialized_relocate(source, source + 3, dest);
...
void test_unique_ptr_backward()
{
    ...
    auto result = std::uninitialized_relocate(
