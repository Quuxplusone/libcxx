...
{
    {
    std::pair<const int, const long> arr[] = { {1,1L}, {2,2L}, {1,1L}, {INT_MAX,1L}, {3,1L} };
    std::multimap m(std::begin(arr), std::end(arr));

    ASSERT_SAME_TYPE(decltype(m), std::multimap<int, const long>);
    std::pair<const int, const long> expected_m[] = { {1,1L}, {1,1L}, {2,2L}, {3,1L}, {INT_MAX,1L} };
    ...
    std::pair<const int, const long> expected_m[] = { {INT_MAX,1L}, {3,1L}, {2,2L}, {1,1L}, {1,1L} };
    assert(std::equal(m.begin(), m.end(), std::begin(expected_m), std::end(expected_m)));
    }

    {
    using P = std::pair<const int, const long>;
    std::pair<const int, const long> arr[] = { {1,1L}, {2,2L}, {1,1L}, {INT_MAX,1L}, {3,1L} };
    std::multimap m(std::begin(arr), std::end(arr), std::greater<int>(), test_allocator<P>(0, 42));

    ASSERT_SAME_TYPE(decltype(m), std::multimap<int, const long, std::greater<int>, test_allocator<P>>);
    std::pair<const int, const long> expected_m[] = { {INT_MAX,1L}, {3,1L}, {2,2L}, {1,1L}, {1,1L} };
    assert(std::equal(m.begin(), m.end(), std::begin(expected_m), std::end(expected_m)));
    ...
    {
    using P = std::pair<const int, long>;
    std::multimap m({ P{1,1L}, P{2,2L}, P{1,1L}, P{INT_MAX,1L}, P{3,1L} }, std::greater<int>(), test_allocator<P>(0, 43));
    ...
    {
    using P = std::pair<const int, long>;
    std::multimap m({ P{1,1L}, P{2,2L}, P{1,1L}, P{INT_MAX,1L}, P{3,1L} }, test_allocator<P>(0, 45));
    ...
    int i = 1;
    int j = 2;
    std::multimap m{ P{i,1L}, P{j,2L} };

    ASSERT_SAME_TYPE(decltype(m), std::multimap<const int&, long, std::less<const int&>>);
    }
    ...
    int i = 1;
    int j = 2;
    std::multimap m({ P{i,1L}, P{j,2L} }, test_allocator<P>(0, 45));

    ASSERT_SAME_TYPE(decltype(m), std::multimap<const int&, long, std::less<const int&>, test_allocator<P>>);
    assert(m.get_allocator().get_id() == 45);
    }
