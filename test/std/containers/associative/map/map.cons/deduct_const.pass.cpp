...
int main(int, char**)
{
    ...
    std::pair<const int, const long> expected_m[] = { {1,1L}, {2,2L}, {3,1L}, {INT_MAX,1L} };
    ...
    std::pair<const int, const long> expected_m[] = { {INT_MAX,1L}, {3,1L}, {2,2L}, {1, 1L} };
    assert(std::equal(m.begin(), m.end(), std::begin(expected_m), std::end(expected_m)));
    }
    ...
    ASSERT_SAME_TYPE(decltype(m), std::map<int, const long, std::greater<int>, test_allocator<P>>);
    std::pair<const int, const long> expected_m[] = { {INT_MAX,1L}, {3,1L}, {2,2L}, {1, 1L} };
    assert(std::equal(m.begin(), m.end(), std::begin(expected_m), std::end(expected_m)));
    ...
    {
    using P = std::pair<const int, long>;
    std::map m({ P{1,1L}, P{2,2L}, P{1,1L}, P{INT_MAX,1L}, P{3,1L} }, std::greater<int>(), test_allocator<P>(0, 43));
    ...
    {
    using P = std::pair<const int, long>;
    std::map m({ P{1,1L}, P{2,2L}, P{1,1L}, P{INT_MAX,1L}, P{3,1L} }, test_allocator<P>(0, 45));
