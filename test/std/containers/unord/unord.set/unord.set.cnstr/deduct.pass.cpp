...
int main(int, char**)
{
    ...
    std::unordered_set s(source); // set(set &)
    ...
    {
    std::unordered_set<int, std::hash<short>, std::equal_to<>, test_allocator<int>> source;
    std::unordered_set s{source, test_allocator<int>(0, 42)}; // set(set &)
    ASSERT_SAME_TYPE(decltype(s), decltype(source));
    assert(s.size() == 0);
    ...
    {
    std::pmr::unordered_set<long> source;
    std::unordered_set s(source, std::pmr::new_delete_resource());
    ASSERT_SAME_TYPE(decltype(s), decltype(source));
    assert(s.size() == 0);
    }
